...
{
    public class StructSerializationInfoFactory : ISerializationFactory<StructSerializationInfo>
    {
        public StructSerializationInfo Create(TypeDefinition definition)
        {
            if (StructSerializationInfo.TryParse(definition, false, out var answer))
            {
                return answer;
            }

...
{
    public static class 二分探索Helper
    {
        public static Instruction[] 二分探索終端Long<TSorter>(in AutomataOption options, BinaryFieldDestinationTuple[] tuples, int tuplesOffset, int tuplesCount, TSorter sorter, VariableDefinition number)
            where TSorter : ILengthSorter
        {
            ...
            switch (tuplesCount)
            {
                ...
                default:
                    ...
                    var 小さい方での二分探索結果 = 二分探索終端Long(options, tuples, tuplesOffset, 小さい方でのtuplesの残存長さ, sorter, number);
                    var 大きい方でのtuplesの残存長さ = tuplesCount - (tuplesCount >> 1) - 1;
                    var 大きい方でのtuplesのoffset = tuplesOffset + 1 + (tuplesCount >> 1);
                    var 大きい方での二分探索結果 = 二分探索終端Long(options, tuples, 大きい方でのtuplesのoffset, 大きい方でのtuplesの残存長さ, sorter, number);
        ...
        public static Instruction[] 二分探索終端Int<TSorter>(in AutomataOption options, BinaryFieldDestinationTuple[] tuples, int tuplesOffset, int tuplesCount, TSorter sorter, VariableDefinition number)
            where TSorter : ILengthSorter
        {
            var failInstruction = options.FailInstruction;
            ref readonly var middleTuple = ref tuples[tuplesOffset + (tuplesCount >> 1)];
            ...
            switch (tuplesCount)
            {
                ...
                default:
                    var 小さい方でのtuplesの残存長さ = tuplesCount >> 1;
                    var 小さい方での二分探索結果 = 二分探索終端Int(options, tuples, tuplesOffset, 小さい方でのtuplesの残存長さ, sorter, number);
                    var 大きい方でのtuplesの残存長さ = tuplesCount - (tuplesCount >> 1) - 1;
                    var 大きい方でのtuplesのoffset = tuplesOffset + 1 + (tuplesCount >> 1);
                    var 大きい方での二分探索結果 = 二分探索終端Int(options, tuples, 大きい方でのtuplesのoffset, 大きい方でのtuplesの残存長さ, sorter, number);
                    var 中央探索結果 = new[]
                    {
                        InstructionUtility.LoadVariable(number),
                        InstructionUtility.LdcI4(middleValue),
